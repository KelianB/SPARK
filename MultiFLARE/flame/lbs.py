...
"""
This is initially taken from the GaussianAvatars repository:
https://github.com/ShenhanQian/GaussianAvatars/blob/main/flame_model/lbs.py
...
def lbs_pose_only(
    ...
    if pose2rot:
        ...
        pose_feature = (pose[:, 1:].view(batch_size, -1, 3, 3) - ident).view(batch_size, -1)
...
def forward_skinning(
    ...
    """Perform the forward skinning part of the `lbs` method, given precomputed transformations and pose features."""

    # Taken from code of `lbs` function
    batch_size = pose.shape[0]
    device = pose.device
    num_joints = lbs_weights.shape[1]

    # Expression blendshapes
    v_shaped = v_canonical + blend_shapes(expr, shapedirs)
    # Pose blendshapes
    v_posed = v_shaped + torch.matmul(pose_feature, posedirs).view(batch_size, -1, 3)
...
def invert_lbs(
    ...
    """Invert the linear blend skinning pose transformation."""
    batch_size, num_points, _ = pnts_posed.shape
    device = pnts_posed.device

    # Do skinning
    # calculate transformation per vertex, using the LBS weights
    ...
    # pnts_p: num_points, 4
    pnts_posed = torch.cat([pnts_posed, homogen_coord], dim=2)
    # v_homo: [num_points, 4, 4] x [num_points, 4, 1] --> [num_points, 4, 1]
    v_homo = torch.matmul(torch.inverse(T), torch.unsqueeze(pnts_posed, dim=-1))
    # pnts: [num_points, 3]
    pnts_c = v_homo[:, :, :3, 0]

    # Revert the pose correctives
    pose_offsets = torch.matmul(pose_feature, posedirs).view(batch_size, -1, 3)
    assert pose_offsets.shape == pnts_c.shape
    return pnts_c - pose_offsets
